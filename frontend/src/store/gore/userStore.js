...
export const useUserStore = defineStore("userStore", () => {
  ...
  const listUsers = async () => {
    ...
    try {
      const response = await api.get("gore/gore_user/");
  ...
  const createUser = async (userData) => {
    try {
      ...
      const response = await api.post("gore/gore_user/", dataToSend);
  ...
  const updateUser = async (id, userData) => {
    try {
      const { password2, ...dataToSend } = userData;
      const response = await api.patch(`gore/gore_user/${id}/`, dataToSend);
  ...
  const deleteUser = async (id) => {
    try {
      await api.delete(`gore/gore_user/${id}/`);
  ...
  const toggleUserStatus = async (id, newStatus) => {
    try {
      ...
      await api.post(`gore/gore_user/${id}/${endpoint}/`);
